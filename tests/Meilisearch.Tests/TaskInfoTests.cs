...
public abstract class TaskInfoTests<TFixture> : IAsyncLifetime where TFixture : IndexFixture
{
    ...
    {
        await _index.AddDocumentsAsync([new Movie { Id = "1" }]);
    ...
    {
        await _index.AddDocumentsAsync([new Movie { Id = "1" }]);
        var taskResponse = await _index.GetTasksAsync(new TasksQuery { Limit = 1, IndexUids = [_index.Uid] });
    ...
    {
        var task = await _index.AddDocumentsAsync([new Movie { Id = "2" }]);
    ...
    {
        var task = await _index.AddDocumentsAsync([new Movie { Id = "3" }]);
    ...
    {
        var task = await _index.AddDocumentsAsync([new Movie { Id = "4" }]);
    ...
    {
        var task = await _index.AddDocumentsAsync([new Movie { Id = "5" }]);
